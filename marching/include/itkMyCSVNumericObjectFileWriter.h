...
#ifndef itkMyCSVNumericObjectFileWriter_h
#define itkMyCSVNumericObjectFileWriter_h
...
{
/**
 * \class MyCSVNumericObjectFileWriter
 * \brief Writes out numeric itk data objects to a csv file.
 *
 * Updated to allow variable size matrix + determine size of rows/columns
 * -------------------
 * Original Header:
 * -------------------
...
class ITK_TEMPLATE_EXPORT MyCSVNumericObjectFileWriter : public LightProcessObject
{
public:
  ITK_DISALLOW_COPY_AND_MOVE(MyCSVNumericObjectFileWriter);

  /** Standard class type aliases */
  using Self = MyCSVNumericObjectFileWriter;
  ...
  using itkVariableSizeMatrixType = itk::VariableSizeMatrix<TValue>;
...
//  SetInput(const itkMatrixType * obj);
  ...
  SizeValueType GetRowHeaderSize();
  SizeValueType GetColumnHeaderSize();
...
protected:
    MyCSVNumericObjectFileWriter();
  ~MyCSVNumericObjectFileWriter() override = default;
  void
...
#  include "itkMyCSVNumericObjectFileWriter.hxx"
#endif
